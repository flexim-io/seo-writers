...
- **Flexim:** the user's request to set up a blog and save this article authorizes those bounded actions and sets `requestedTarget: cms_draft`. Reuse current schema tools and the canonical blog template, then dispatch the existing private-draft handoff. A generic choice such as “I have no CMS” only permits offering Flexim; it does not authorize writes. Keep the finished package available if access or setup fails.
